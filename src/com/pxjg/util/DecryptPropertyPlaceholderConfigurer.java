			...
			for (int i = 0; i < this.locations.length; i++) {
				...
				try {
					...
					// 属性文件
						...
						this.propertiesPersister.doLoad(props,
								new InputStreamReader(is, this.fileEncoding));
						String content = this.propertiesPersister
								.getEncryptContent();
						// 查找是否存在加密标识
						if (StringUtils.contains(content, "{DES}")) {
							try {
								File file = location.getFile();
								fos = new FileOutputStream(file);
								fos.write(this.propertiesPersister
										.getEncryptContent().getBytes());
								fos.flush();
							} finally {
								if (null != fos) {
									fos.close();

        		...
        		<table cellpadding="0" cellspacing="0" width="100%">
                	<tr>
	             		<td align="right" colspan="5">
	             			机构新闻查询<input type="text" name="search" id="search" /><input type="button" onclick="onsearch();" value="查询" />
	             		</td>
	             	</tr>
	             </table>
                	...
                	<table cellpadding="0" cellspacing="0" width="100%">
                      <tr>
                        ...
                        <th scope="col" width="266">排序</th>
                         <th scope="col" width="340">操作</th>
                      </tr>
                      <s:iterator value="nList">
							...
							<td><s:property value="state==0?'未激活':'已激活'"/></td>
                 ...
                 <div >
                    	<jsp:include page="../page.jsp">
							<jsp:param name="url" value="<%=path+"/newsAction!newsDeptListByAdmin.do"%>"/>
						</jsp:include>
                    </div>
...
<script type="text/javascript">
	...
	function goUpdate(id){
		var url = "<%=path %>/newsAction!goUpdateDeptNewsByAdmin.do?id="+id;
		window.location.href = url;
	}
	
	function onsearch(){
		var search = $("#search").val();
		var url = "<%=path %>/newsAction!newsDeptListAdmin.do?search="+search;
		window.location.href = url;
	}
	     	...
	     	url: "newsAction!delete.do?id="+id,
	      	success: function(msg){
	      		 if(msg==0){
	      		 	...
	      		 	alert("删除成功");
	      		 	var url = "<%=path%>/newsAction!newsDeptListAdmin.do";
	        	 	window.location.href = url;
	      		 }
	     		}
	  		});
	...
	function openOrClose(id,state){
		if(confirm("确定激活吗？")){
			$.ajax({
	      	type: "POST",
	     	url: "newsAction!openOrClose.do?id="+id+"&state="+state,
	      	success: function(msg){
	      		 if(msg==0){
	      		 	alert("操作失败");
	      		 }else if(msg==1){
